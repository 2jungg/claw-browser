...
impl<'a> LayoutBox<'a> {
    ...
    pub fn layout(&mut self, containing_block: Dimensions) {
        match self.box_type {
            ...
            BoxType::InlineNode(_) => {}, // Not implemented
            BoxType::AnonymousBlock => {}, // Not implemented
        }
    }

    fn layout_block(&mut self, containing_block: Dimensions) {
        // Child width depends on parent width, so we need to calculate this box's width before its children's.
        self.calculate_block_width(containing_block);

        // Finish calculating the box's position.
        self.calculate_block_position(containing_block);

        // Recursively lay out the children of this box.
        self.layout_block_children();

        // Parent height depends on child height, so we calculate this box's height after its children's.
    ...
    fn layout_block_children(&mut self) {
        ...
        for child in &mut self.children {
            child.layout(*d);
            // Increment the height of the container so that the next child is placed below.
            d.content.height += child.dimensions.margin_box().height;
        }
    }

    fn calculate_block_height(&mut self) {
        // If the height is set to an explicit length, use that. Otherwise, use the height calculated by `layout_block_children`.
